...
function countTreeNodesRecursive(node) {
    if (!node) {
      ...
      return 1 + countTreeNodesRecursive(node.left) + countTreeNodesRecursive(node.right)
    }
}
...
function validateBST(tree) {
    return validateRecur(tree)
}

function validateRecur(node, min = -Infinity, max = Infinity) {
    if (!node) return true
    
    if (node.value <= min || node.value >= max) return false
  
    return (
      validateRecur(node.left, min, node.value) &&
      validateRecur(node.right, node.value, max)
    )
}
...
function fibonacci(n, hist = {}) {
    if (hist[n]) return hist[n]
    
    if (n <= 1) return n
    
    return hist[n] = fibonacci(n - 1, hist) + fibonacci(n - 2, hist)
}
